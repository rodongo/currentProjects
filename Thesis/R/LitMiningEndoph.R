...
endos <- c('tau','amyloid','immun','inflammation',
           'oxidat','lysosom','vascular','endothelial',
           'mito','proteostasis','synap')
endosNames <- c('Tauopathy','Amyloidopathy','Neuroinflammation','Neuroinflammation2',
           'Oxidative Stress','Lysosomal Dysfunction','Vascular Dysfunction','Vascular Dysfunction2',
...
for(g in 1:length(endos)){
  ...
  for(h in 1:length(adNeigh)){
    print(adNeigh[h])
    # Sys.sleep(0.50)
    my_query <- paste('[Alzheimer] disease AND [',endos[g],']', 'AND[', adNeigh[h],']gene', sep = ' ')
    my_query <- get_pubmed_ids(my_query, api_key = api_key)
    if(length(my_query$IdList)>0){
...
endosList22.1 <- lapply(endosList22, function(x){x = x %>% mutate(Evidence = as.numeric(x$`Number of Publications`)*100/max(as.numeric(x$`Number of Publications`)),
                                                                  Gene = toupper(x$`AD Gene`)) %>% select(-`AD Gene`)})
endosList22.1 <- lapply(endosList22.1, function(x){x = x %>% filter(.$Evidence > 30)})
endosList22.1 <- lapply(endosList22.1, function(x){x = x[!duplicated(x$Gene),]})

endosList22 %>% writexl::write_xlsx(path = 'AD_Endophen_PubMed2.xlsx')
endosList22.1 %>% writexl::write_xlsx(path = 'AD_Endophenotype2Gene.xlsx')
